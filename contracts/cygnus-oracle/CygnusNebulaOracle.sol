...
contract CygnusNebulaOracle is ICygnusNebulaOracle, ReentrancyGuard {
     ...
     *  @inheritdoc ICygnusNebulaOracle
     */
    string public constant override name = "Cygnus-Nebula: MetaStable BPT Oracle";
    ...
    function lpTokenPriceUsd(address lpTokenPair) external view override returns (uint256 lpTokenPrice) {
        ...
        // 1. Get invariant (always 18 decimals)
        (uint256 invariant, ) = IMetastablePool(cygnusNebula.underlying).getLastInvariant();

        // 2. Get min price of assets
        uint256 minPrice = type(uint256).max;

        // Loop through each token price
        for (uint256 i = 0; i < cygnusNebula.poolTokens.length; i++) {
            // Asset price to 18 decimals
            uint256 assetPrice = getPriceInternal(cygnusNebula.priceFeeds[i]);

            // Update min price
            minPrice = minPrice < assetPrice ? minPrice : assetPrice;
        }

        // 3. Get total Supply
        uint256 totalSupply = IMetastablePool(cygnusNebula.underlying).totalSupply();

        // 4. BPT Price (USD) = TVL / totalSupply
        uint256 lpPrice = PRBMath.mulDiv(minPrice, invariant, totalSupply);
